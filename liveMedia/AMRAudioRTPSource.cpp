...
public:
  ...
  createNew(UsageEnvironment& env,
	    ...
	    Boolean isWideband, Boolean isInterleaved, Boolean CRCsArePresent);
...
private:
  Boolean fIsWideband, fIsInterleaved, fCRCsArePresent;
...
AMRAudioRTPSource::createNew(UsageEnvironment& env,
  ...
  // We don't yet support some modes:
  if (!isOctetAligned) {
    env << "AMRAudioRTPSource::createNew(): Bandwidth-efficient mode was specified, but we don't yet support this!\n";
    return NULL;
  } else if (robustSortingOrder) {
  ...
  resultRTPSource = rawRTPSource
    = RawAMRRTPSource::createNew(env, RTPgs, rtpPayloadFormat,
				 isWideband, isInterleaved, CRCsArePresent);
...
RawAMRRTPSource::createNew(UsageEnvironment& env, Groupsock* RTPgs,
			   unsigned char rtpPayloadFormat,
			   Boolean isWideband, Boolean isInterleaved,
			   Boolean CRCsArePresent) {
...
::RawAMRRTPSource(UsageEnvironment& env,
  ...
  fIsWideband(isWideband), fIsInterleaved(isInterleaved),
...
static unsigned short frameSizeFromFT[16] = {
  12, 13, 15, 17,
  19, 20, 26, 31,
  5, FT_INVALID, FT_INVALID, FT_INVALID,
  FT_INVALID, FT_INVALID, FT_INVALID, 0
};
static unsigned short frameSizeFromFTWideband[16] = {
  17, 23, 32, 36,
  40, 46, 50, 58,
...
unsigned AMRBufferedPacket::
  ...
  unsigned short frameSize
    = fOurSource.isWideband() ? frameSizeFromFTWideband[FT] : frameSizeFromFT[FT];
